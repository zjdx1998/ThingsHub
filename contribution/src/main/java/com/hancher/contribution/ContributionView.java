...
//        widthSize = widthSize + getPaddingLeft() + getPaddingRight();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (i == selectItem) {
                canvas.drawRoundRect(rect, config.getItemRound(), config.getItemRound(), itemPaint);//config.getSelectPaint());
            } else {
                canvas.drawRoundRect(rect, config.getItemRound(), config.getItemRound(), itemPaint);//getColorPaintByNumber(item));
            }
